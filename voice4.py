"""
JARVIS Voice System - MERGED EDITION
Hollywood quality + No word restrictions + AI pronunciation
"""
...
class PhonemeEngine:
    ...
    def word_to_phonemes(cls, word):
        """Convert any word to phonemes - AI style prediction"""
        ...
        # AI-based pronunciation prediction
        phonemes = []
        i = 0
        length = len(word)
        ...
        while i < length:
            ...
            if char in 'aeiou':
                ...
                if i < length - 1 and word[i+1] == 'e' and i < length - 2:
                    if word[i+2] in 'mnrt':
                        # Magic e pattern
                        if char == 'a':
                            phonemes.append('ey')
            ...
            # Handle consonants
            elif char in cls.LETTER_TO_PHONEME:
                cons_phonemes = cls.LETTER_TO_PHONEME[char]
                phonemes.extend(cons_phonemes)
...
class HollywoodVoiceEngine:
    """Professional voice engine with Hollywood quality"""
    ...
    def generate_glottal_pulse(self, duration, f0, config):
        """Generate glottal pulse with harmonics"""
        ...
        # Generate pulse with harmonics
        pulse = np.zeros(n_samples)
        for h in range(1, 5):  # 4 harmonics
            amp = 1.0 / h
    ...
    def generate_phoneme(self, phoneme, duration, config):
        ...
        if phoneme_data['type'] == 'vowel':
            ...
            if phoneme_data['type'] == 'plosive':
                ...
                source = self.generate_gloottal_pulse(duration, config.base_f0, config)
                sound = source * 0.3
...
class JarvisVoiceSystem:
    """Complete voice system - Hollywood quality, any word"""
    ...
    def text_to_phonemes(self, text):
        """Convert ANY text to phonemes - AI based"""
    ...
    def generate_speech(self, text, output_file=None):
        ...
        # Convert to phonemes (AI predicts any word)
    ...
    def list_voices(self):
        ...
        for name, config in self.presets.items():
            print(f"  • {name:10} ({config.gender}, {config.effect_type})")
...
class VoiceConsole:
    ...
    def run(self):
        """Run console"""
        print("\n" + "="*60)
        print("🎙️ JARVIS HOLLYWOOD VOICE SYSTEM")
        print("   Any word - AI Pronunciation")
        print("="*60)
        print("\nCommands:")
        ...
        print("  /exit      - Exit")
        print("-" * 40)
        print("Type anything - JARVIS will speak it!")
